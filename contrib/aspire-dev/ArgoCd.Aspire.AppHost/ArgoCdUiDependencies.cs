...
public static class ArgoCdUiDependencies
{
    ...
    public static void EnsureInstalledOrThrow(string uiDir, TimeSpan timeout)
    {
        ...
        var args = pnpm.Arguments.Concat(["install", "--registry=https://registry.npmjs.org/"]).ToArray();
        ...
        if (exitCode != 0)
        {
            throw new InvalidOperationException(
                $"'corepack pnpm install --registry=https://registry.npmjs.org/' failed in '{uiDir}' (exit code {exitCode}).{Environment.NewLine}" +
                ...
                $"or install dependencies manually with 'corepack pnpm install --registry=https://registry.npmjs.org/' in '{uiDir}'; " +
