    ...
    : IDistributedApplicationEventingSubscriber, IAsyncDisposable
{
    ...
    public Task AfterResourcesCreatedAsync(DistributedApplicationModel appModel, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            repoRoot = ArgoCdRepoRoot.Resolve(AppContext.BaseDirectory);
        }
        catch (DirectoryNotFoundException ex)
        {
            logger.LogWarning(
                ex, "Could not locate the repository root; automatic selective restart on Go source edits is disabled for this run.");
            return Task.CompletedTask;
        }
