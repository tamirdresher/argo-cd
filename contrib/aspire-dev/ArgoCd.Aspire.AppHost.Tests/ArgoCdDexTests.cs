...
{
    private static readonly string FakeRepoRoot = Path.Combine(Path.GetTempPath(), "argocd-aspire-dex-tests-repo");
    ...
    public async Task GenDexConfig_MatchesProcfileCommandAndFlags()
    {
        ...
        var gendexcfg = builder.Builder.AddArgoCdGenDexConfig(FakeRepoRoot, dexConfigPath, cluster);

        Assert.Equal("go", gendexcfg.Resource.Command);
        Assert.Equal(FakeRepoRoot, gendexcfg.Resource.WorkingDirectory);
